...
CC=${ANDROID_NDK_HOME}/toolchains/llvm/prebuilt/linux-x86_64/bin/aarch64-linux-android21-clang GOARCH=arm64 go build -ldflags='-s -w -buildid=' -tags ${BUILD_TAGS} -trimpath -o ${current}/arm64-v8a/libtailscaled.so -v ./cmd/tailscaled/.
CC=${ANDROID_NDK_HOME}/toolchains/llvm/prebuilt/linux-x86_64/bin/x86_64-linux-android21-clang GOARCH=amd64 go build -ldflags='-s -w -buildid=' -tags ${BUILD_TAGS} -trimpath -o ${current}/x86_64/libtailscaled.so -v ./cmd/tailscaled/.
